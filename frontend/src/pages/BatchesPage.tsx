...
export default function BatchesPage() {
  ...
  useEffect(() => {
    if (!focusCode || !rows.some(b => b.code === focusCode)) return;
    const el = document.getElementById(`batch-row-${focusCode}`);
  ...
  return (<>
    ...
    <tbody>{rows.map(b => <tr key={b.id} id={`batch-row-${b.code}`} className={b.code === focusCode ? "batch-row--hot" : undefined}><td className="mono">{b.code}</td><td>{b.product_name}</td><td>{b.oven_label}</td>
